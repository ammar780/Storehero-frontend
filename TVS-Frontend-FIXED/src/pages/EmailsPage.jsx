    ...
    <div>
      ...
      {loading ? <><SkeletonCards count={6}/><SkeletonTable/></> : (<>
        ...
        {tab === 'overview' && (<div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-surface-50 to-purple-50 dark:from-surface-800 dark:to-purple-900/20 rounded-2xl border border-surface-200 p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 text-center">
              <div><div className="text-xs text-surface-400 uppercase mb-1">Total Sent</div><div className="text-2xl font-display font-bold">{fmt.number(s.total_sends||0)}</div><div className="text-xs text-surface-400">EmailIt: {fmt.number(s.emailit_sends||0)} · EM: {fmt.number(s.enginemailer_sends||0)}</div></div>
        ...
        {tab === 'campaigns' && (<div className="space-y-6">
          ...
          {/* EmailIt Campaign Stats (from tracking plugin) */}
          {perfData?.emailitCampaigns?.length > 0 && (
            <div className="bg-gradient-to-r from-rose-50 to-orange-50 dark:from-rose-900/20 dark:to-orange-900/20 border border-rose-200 dark:border-rose-800 rounded-2xl p-5 mb-6">
              <h3 className="font-display font-bold text-sm mb-3">EmailIt Campaigns (Transactional — from tracking plugin)</h3>
              <DataTable columns={[
                ...
                {key:'unique_opens',label:'Opens',align:'right',render:v=>fmt.number(v)},
                {key:'clicks',label:'Clicks',align:'right',render:v=>fmt.number(v)},
                {key:'sends',label:'Open %',align:'right',render:(v,row)=>{const r=+v>0?Math.round(+row.unique_opens/+v*1000)/10:0; return <span className={`font-mono font-bold ${r>30?'text-emerald-600':r>15?'text-blue-600':r>5?'text-amber-600':'text-red-600'}`}>{r}%</span>}},
                {key:'sends',label:'Placement',align:'center',render:(v,row)=>{const r=+v>0?+row.unique_opens/+v*100:0; return healthBadge(r>30?'good':r>15?'average':r>5?'low':r>0?'poor':'likely_spam')}},
              ]} data={perfData.emailitCampaigns} searchable={['campaign']} />
            </div>
          )}
        ...
        {tab === 'emails' && (<div className="space-y-6">
          {perfData?.emailitEmails?.length > 0 && (
            <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5 mb-6">
              <h3 className="font-display font-bold text-sm mb-3">EmailIt Individual Emails (Transactional)</h3>
              <DataTable columns={[
                {key:'campaign',label:'Campaign',render:v=><span className="text-xs">{clean(v)}</span>},
                {key:'email_id',label:'Email',render:v=><span className="font-semibold text-sm">{clean(v)}</span>},
                {key:'sends',label:'Sent',align:'right',render:v=>fmt.number(v)},
                {key:'unique_opens',label:'Opens',align:'right',render:v=>fmt.number(v)},
                {key:'clicks',label:'Clicks',align:'right',render:v=>fmt.number(v)},
                {key:'sends',label:'Open %',align:'right',render:(v,row)=>{const r=+v>0?Math.round(+row.unique_opens/+v*1000)/10:0; return <span className={`font-mono font-bold ${r>30?'text-emerald-600':r>15?'text-blue-600':r>5?'text-amber-600':'text-red-600'}`}>{r}%</span>}},
                {key:'sends',label:'Placement',align:'center',render:(v,row)=>{const r=+v>0?+row.unique_opens/+v*100:0; return healthBadge(r>30?'good':r>15?'average':r>5?'low':r>0?'poor':'likely_spam')}},
              ]} data={perfData.emailitEmails} searchable={['campaign','email_id']} />
            </div>
          )}
