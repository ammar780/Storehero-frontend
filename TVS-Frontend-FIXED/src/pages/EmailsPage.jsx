    ...
    <div className="border border-surface-200 dark:border-surface-700 rounded-xl overflow-hidden mb-2">
      ...
      {open && (
        <div className="border-t border-surface-100 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50 p-4">
          ...
          {camp.matchedUtmCampaigns?.length > 0 && (
            <div className="mt-3">
              <div className="text-xs text-surface-400 mb-1">Sales from UTM campaigns:</div>
              {(camp?.matchedUtmCampaigns||[]).map((s,j) => (
                <div key={j} className="flex items-center gap-2 text-xs bg-white dark:bg-surface-700 rounded-lg p-2 mt-1">
                  <span className="font-mono bg-purple-100 dark:bg-purple-900/30 px-2 py-0.5 rounded">{s.campaign}</span>
                  {s.cta && s.cta !== 'unknown' && <span className="text-surface-400">CTA: {s.cta}</span>}
                  <span className="ml-auto font-bold text-emerald-600">{s.orders} orders · ${Math.round(s.revenue)}</span>
                </div>
              ))}
            </div>
          )}
          {camp.emails > 0 && !camp.matchedUtmCampaigns?.length && (
            <div className="mt-3 text-xs text-surface-400">
              This campaign has {camp.emails} emails. Sales will appear when customers click UTM-tagged email links and buy.
            </div>
          )}
        </div>
      )}
...
export default function EmailsPage() {
  ...
  const { data: allCamps, loading: l1 , error } = useApi('/analytics/all-campaigns', { period }, [period]);
  ...
  const emailSales = allCamps?.totalEmailSales || {};
    ...
    <div>
      ...
      {loading ? <><SkeletonCards count={4}/><SkeletonTable/></> : (<>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 stagger">
          <MetricCard label="Total Sent" value={totalSends || +(s.total_sends||0)} format="number" icon={Send} color="bg-surface-50 dark:bg-surface-700 text-surface-600" />
        ...
        {tab === 'campaigns' && (
          <div className="space-y-6">
            ...
            {allCamps?.unmatchedCount > 0 && (
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 rounded-xl p-3 mb-4">
                <div className="text-xs font-semibold text-amber-700">{allCamps.unmatchedCount} UTM campaign(s) from orders didn't match any predefined campaign — shown below under "Unmatched".</div>
              </div>
            )}
            {campaigns.length === 0 && (
        ...
        {tab === 'funnel' && (
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-surface-50 to-purple-50 dark:from-surface-800 dark:to-purple-900/20 rounded-2xl border p-4 sm:p-6">
