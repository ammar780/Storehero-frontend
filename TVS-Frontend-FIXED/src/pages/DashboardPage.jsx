...
import { DollarSign, TrendingUp, ShoppingCart, Users, Target, ArrowUpDown, BarChart3, RefreshCw, CheckCircle, Circle, ArrowRight, Zap, Brain, Activity, AlertTriangle, Bell, Shield, } from 'lucide-react';

function LiveTicker({ data }) {
  if (!data) return null;
  const t = data.today || {};
  const y = data.yesterday || {};
  const revPct = +y.revenue > 0 ? (((+t.revenue - +y.revenue) / +y.revenue * 100)||0).toFixed(0) : 0;
  return (
    <div className="bg-gradient-to-r from-brand-500 to-brand-600 rounded-2xl p-4 mb-6 text-white flex flex-wrap items-center gap-4 sm:p-6">
      ...
      {+revPct !== 0 && <div className={`text-xs font-semibold px-2 py-1 rounded-lg ${+revPct > 0 ? 'bg-white dark:bg-surface-800/20' : 'bg-red-500/30'}`}>{+revPct > 0 ? '↑' : '↓'} {Math.abs(revPct)}% vs yesterday</div>}
    </div>
  );
}
  ...
  return (
    <div className="bg-gradient-to-r from-brand-50 to-brand-100/50 border border-brand-200 rounded-2xl p-5 mb-6">
      ...
      <div className="space-y-2.5">
        {(steps||[]).map((s, i) => (
          <a key={i} href={s.link} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${s.done ? 'bg-white dark:bg-surface-800/50' : 'bg-white dark:bg-surface-800 shadow-sm hover:shadow-card'}`}>
...
export default function DashboardPage() {
  const { data: aiBrief } = useApi('/ai/daily-brief', {}, []);
  const [period, setPeriod] = useState('12m');
  const { data, loading, error, refetch } = useApi('/dashboard/overview', { period });
  ...
  const metrics = [
    ...
    { label: 'Tax Collected', value: c.tax_total, prev: p?.tax_total, format: 'currency', icon: Shield, color: 'text-red-500' },
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-white dark:bg-surface-800/95 backdrop-blur-sm border border-surface-200 dark:border-surface-700 rounded-xl px-3 py-2 shadow-lg text-xs">
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={doRefresh} className="p-2.5 rounded-xl bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:bg-surface-700 transition-all">
      ...
      {data?.lastSyncAt && <div className="text-xs text-surface-400 mb-2">Last synced: {new Date(data.lastSyncAt).toLocaleString()}</div>}
      ...
      {loading ? <SkeletonCards count={8} /> : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6 stagger">
          {metrics.map((m, i) => (
            <MetricCard key={i} label={m.label} value={m.value} prev={m.prev} format={m.format} icon={m.icon} iconColor={m.color} delay={i * 0.04} />
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        {/* Revenue Trend */}
        <div className="lg:col-span-2 bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
