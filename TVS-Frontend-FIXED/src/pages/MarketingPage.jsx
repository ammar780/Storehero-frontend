...
export default function MarketingPage() {
  ...
  return (
    <div>
      ...
      {loading ? <><SkeletonCards count={4}/><SkeletonTable/></> : (!data?.byPlatform?.length && !data?.revVsAds?.length ? (
        <div className="text-center py-16 text-surface-400 dark:text-surface-500"><DollarSign size={48} className="mx-auto mb-4 opacity-30"/><div className="text-lg font-display font-bold mb-2">No ad spend data yet</div><p className="text-sm">Connect Meta, Google, or TikTok Ads in Settings, then sync.</p></div>
      ) : (<>
        ...
        {data?.byPlatform?.length > 0 && (
          <DataTable columns={[
            ...
            {key:'roas',label:'ROAS',align:'right',render:v=><span className={`font-mono font-bold ${+v>=3?'text-emerald-600':+v>=1?'text-blue-600':'text-red-600'}`}>{((+v)||0).toFixed(1)}x</span>},
