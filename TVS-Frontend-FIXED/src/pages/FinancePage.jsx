...
function CompRow({ label, curr, prev, indent, bold, border }) {
  ...
  return (
    <tr className={`${border ? 'border-t-2 border-surface-200 dark:border-surface-700' : 'border-b border-surface-50'} ${bold ? 'font-bold' : ''}`}>
      ...
      <td className={`py-2.5 px-4 text-right text-xs font-semibold ${Math.abs(ch) < 1 ? 'text-surface-400' : better ? 'text-emerald-600' : 'text-red-500'}`}>
        {Math.abs(ch) < 0.1 ? '—' : `${ch > 0 ? '+' : ''}${(+(ch)||0).toFixed(1)}%`}
      </td>
    </tr>
  );
}
...
export default function FinancePage() {
  const { data: channelData } = useApi('/analytics/channel-margins', { period: '30d' }, []);
  const exportPnl = () => { const url = (api.defaults.baseURL||'') + '/finance/pnl-export?period=' + period; window.open(url, '_blank'); };
  const [period, setPeriod] = useState('12m');
  const [tab, setTab] = useState('pnl');
  const { data: comp, loading: lc , error } = useApi('/dashboard/pnl-comparison', { period }, [period]);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="inline-flex bg-white dark:bg-surface-800 rounded-xl border border-surface-200 dark:border-surface-700 p-0.5">
          ...
          <button onClick={exportPnl} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold">Export P&L</button>
        ...
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 overflow-hidden">
          ...
          {lc ? <SkeletonTable /> : (
            <table className="w-full">
              <thead><tr className="border-b border-surface-100 bg-surface-50 dark:bg-surface-700/50">
              ...
              <tbody>
                ...
                <CompRow label="Store Credits" curr={c.store_credits} prev={p.store_credits} indent />
                <CompRow label="Sales Tax Remitted" curr={c.tax_total||c.sales_tax} prev={p.tax_total||p.sales_tax} indent />
                <CompRow label="Affiliate Commissions" curr={c.affiliate_commissions} prev={p.affiliate_commissions} indent />
