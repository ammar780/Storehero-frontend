...
function OrderModal({ order, onClose }) {
  const { data, loading , error } = useApi(order ? `/orders/${order.id}` : null, {}, [order?.id]);
  if (!order) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-surface-800 rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto p-4 sm:p-6" onClick={e=>e.stopPropagation()}>
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        ...
        <span className="border-l border-surface-200 dark:border-surface-700 mx-1"/>
      ...
      {loading ? <SkeletonTable /> : (!orders.length ? (
        <div className="text-center py-16 text-surface-400 dark:text-surface-500"><ShoppingCart size={48} className="mx-auto mb-4 opacity-30"/><div className="text-lg font-display font-bold mb-2">No orders found</div><p className="text-sm">Try a different time period or sync WooCommerce.</p></div>
      ) : (
        <DataTable columns={[
