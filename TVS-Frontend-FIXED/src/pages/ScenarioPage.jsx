...
export default function ScenarioPage() {
  ...
  // #45 Auto-run on slider change with debounce
  useEffect(() => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => { run(); }, 400);
    return () => clearTimeout(debounceRef.current);
  }, [form.cogs_change_pct, form.ad_spend_change_pct, form.price_change_pct]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:p-6">
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-6 space-y-6">
        ...
        {result && (
          <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-6 animate-scale-in">
            ...
            <div className="space-y-4">
              {[['Revenue',result.base.revenue,result.scenario.revenue],['COGS',result.base.cogs,result.scenario.cogs],['Ad Spend',result.base.ad_spend,result.scenario.ad_spend],['Gross Profit',result.base.gross_profit,result.scenario.gross_profit],['Net Profit',result.base.net_profit,result.scenario.net_profit]].map(([label,base,scenario])=>{
                ...
                return (
                  <div key={label} className="flex items-center justify-between p-3 rounded-xl bg-surface-50 dark:bg-surface-700">
