...
export default function LTVPage() {
  const { data: affinityData } = useApi('/analytics/product-affinity', {}, []);
  const { data: ltv, loading , error } = useApi('/customers/ltv-overview');
  ...
  return (
    <div>
      ...
      {loading ? <><SkeletonCards /><SkeletonTable /></> : (
        <>
          ...
          {ltv?.distribution?.length > 0 && (
            <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-6 mt-6">
      ...
      {/* Product Affinity */}
      {affinityData?.pairs?.length > 0 && (
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5 mt-6">
          <h3 className="font-display font-bold text-sm mb-3">Frequently Bought Together</h3>
          <div className="space-y-2">{(affinityData?.pairs||[]).slice(0,10).map((p,i) => (
            <div key={i} className="flex items-center gap-3 p-2 bg-surface-50 dark:bg-surface-700 rounded-lg">
              <span className="text-sm font-semibold">{p.product_a}</span>
              <span className="text-xs text-surface-400">+</span>
              <span className="text-sm font-semibold">{p.product_b}</span>
              <span className="ml-auto text-xs bg-purple-100 dark:bg-purple-900/30 px-2 py-0.5 rounded">{p.co_purchases}x</span>
            </div>
          ))}</div>
        </div>
      )}
