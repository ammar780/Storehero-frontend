    ...
    <div>
      ...
      <div className="relative">
        ...
        <input type="number" step="0.01" min="0" value={value} onChange={e => onChange(e.target.value)}
          ...
          className="w-full pl-5 pr-1 py-1.5 border border-surface-200 dark:border-surface-700 rounded-lg text-xs font-mono focus:outline-none focus:ring-2 focus:ring-brand-400/30 text-right" />
      </div>
    </div>
  );
}
...
function ProductRow({ product, onSaved }) {
  ...
  const margin = price > 0 && landed > 0 ? (((price - landed) / price * 100)||0).toFixed(1) : null;
...
export default function ProductsPage() {
  const { data, loading, refetch , error } = useApi('/products', { limit: 500 });
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products..." className="w-full pl-9 pr-4 py-2.5 border border-surface-200 dark:border-surface-700 rounded-xl text-sm bg-white dark:bg-surface-800 focus:outline-none focus:ring-2 focus:ring-brand-400/30" />
        </div>
