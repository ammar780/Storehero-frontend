...
const fmtMetric = (key, val) => {
  ...
  if (['mer','ltv_cac_ratio'].includes(key)) return (+(val)||0).toFixed(1) + 'x';
...
export default function BenchmarksPage() {
  ...
  const { data, loading , error } = useApi('/analytics/benchmarks', { period }, [period]);
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonCards count={4} /> : (!comparisons.length ? (
        <div className="text-center py-16 text-surface-400 dark:text-surface-500"><Target size={48} className="mx-auto mb-4 opacity-30"/><div className="text-lg font-display font-bold mb-2">No benchmark data yet</div><p className="text-sm">Sync WooCommerce to calculate your metrics against industry benchmarks.</p></div>
      ) : (<>
        {/* Score card */}
            ...
            <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={(comparisons||[]).slice(0, 8)} layout="vertical">
                  ...
                  <Tooltip formatter={(v, n) => [typeof v === 'number' ? (+(v)||0).toFixed(1) : v, n]} />
