    ...
    <div className={`flex gap-3 ${role === 'user' ? 'justify-end' : ''}`}>
      ...
      {pricingData?.products?.length > 0 && (
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5 mt-6">
          <h3 className="font-display font-bold text-sm mb-3">AI Pricing Recommendations</h3>
          <div className="space-y-2">{pricingData.products.filter(p=>p.suggestion?.action!=='hold').map((p,i) => (
            <div key={i} className={`p-3 rounded-xl border ${p.suggestion.action==='increase_price'?'bg-emerald-50 border-emerald-200':'bg-amber-50 border-amber-200'}`}>
              <div className="font-semibold text-sm">{p.name} — {p.margin}% margin</div>
              <div className="text-xs text-surface-500 mt-1">{p.suggestion.reason}</div>
              {p.suggestion.target && <div className="text-xs font-mono mt-1">Suggested price: ${p.suggestion.target}</div>}
            </div>
          ))}</div>
          {pricingData.products.filter(p=>p.suggestion?.action==='hold').length > 0 && (
            <div className="text-xs text-surface-400 mt-3">{pricingData.products.filter(p=>p.suggestion?.action==='hold').length} products have healthy margins — no changes needed.</div>
          )}
        </div>
      )}
      ...
      {insightsData?.insights?.length > 0 && (
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5 mt-6">
          <h3 className="font-display font-bold text-sm mb-3">This Week's AI Analysis</h3>
          <div className="space-y-2">{insightsData.insights.map((ins,i) => (
            <div key={i} className="flex items-start gap-2 text-sm">
              <span>{ins.type==='positive'?'📈':ins.type==='negative'?'📉':ins.type==='warning'?'⚠️':'💡'}</span>
              <span className="dark:text-surface-300">{ins.text}</span>
            </div>
          ))}</div>
        </div>
      )}
...
function formatAiResponse(text) {
  if (!text) return '';
  return text
    .replace(/^### (.+)$/gm, '<h4 style="color:#f1c349;margin:16px 0 8px;font-size:15px;font-weight:700">$1</h4>')
    .replace(/^## (.+)$/gm, '<h3 style="color:#f1c349;margin:20px 0 10px;font-size:17px;font-weight:700">$1</h3>')
    .replace(/^# (.+)$/gm, '<h2 style="color:#f1c349;margin:24px 0 12px;font-size:19px;font-weight:700">$1</h2>')
    .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
    .replace(/^- (.+)$/gm, '<div style="padding:3px 0 3px 16px;position:relative"><span style="position:absolute;left:0;color:#f1c349">•</span> $1</div>')
