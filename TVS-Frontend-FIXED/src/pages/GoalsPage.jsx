...
export default function GoalsPage() {
  ...
  const { data, loading, refetch , error } = useApi('/dashboard/goals-pacing', { year });
  ...
  const delGoal = async (id) => { if(!window.confirm('Delete this goal?'))return; await api.delete('/settings/goals/' + id); refetchGoals(); refetch(); };
  ...
  return (
    <div>
      ...
      {adding && (
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-6 mb-6 animate-scale-in">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div>
              ...
              <select value={form.metric_type} onChange={e => setForm({...form, metric_type: e.target.value})} className="px-3 py-2.5 border border-surface-200 dark:border-surface-700 rounded-xl text-sm bg-white dark:bg-surface-800 focus:outline-none focus:ring-2 focus:ring-brand-400/30">
            ...
            <div>
              ...
              <input type="number" value={form.annual_target} onChange={e=>setForm({...form,annual_target:e.target.value})} className="px-3 py-2.5 border border-surface-200 dark:border-surface-700 rounded-xl text-sm w-40 focus:outline-none focus:ring-2 focus:ring-brand-400/30" placeholder="e.g. 500000" />
            </div>
      ...
      {loading ? <SkeletonCards count={3} /> : (
        <div className="space-y-6">
          {(goals||[]).map(g => {
            ...
            return (
              <div key={g.id} className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-6 hover:shadow-cardHover transition-shadow">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-6 lg:grid-cols-12 gap-2">
                  {(months||[]).map((m, i) => {
                    ...
                    return (
                      <div key={m} className={`text-center p-2 rounded-xl ${isCurrent ? 'bg-brand-50 ring-2 ring-brand-400/30' : 'bg-surface-50 dark:bg-surface-700'}`}>
