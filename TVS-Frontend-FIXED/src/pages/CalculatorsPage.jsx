...
function CalcCard({ title, description, fields, endpoint, formatResult }) {
  ...
  return (
    <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-6 hover:shadow-cardHover transition-shadow">
        ...
        {fields.map(f => (
          <div key={f.key}>
            ...
            <input type="number" step={f.step||'0.01'} value={form[f.key]} onChange={e=>setForm({...form,[f.key]:e.target.value})}
              className="w-full px-3 py-2 border border-surface-200 dark:border-surface-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-400/30" placeholder={f.placeholder||''} />
          </div>
        ))}
      </div>
