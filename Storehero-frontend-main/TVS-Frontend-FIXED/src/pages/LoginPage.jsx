...
export default function LoginPage() {
    ...
    try {
      if (step === 'credentials') {
        ...
        if (result.otpRequired) {
          setStep('otp');
          setInfo(result.message || 'Check your email for the verification code');
        }
        // If success, AuthContext sets user → App re-renders → dashboard shows
      } else if (step === 'otp') {
  ...
  return (
    <div className="min-h-screen flex px-4 dark:bg-surface-900 items-center justify-center bg-gradient-to-br from-sidebar-950 via-sidebar-900 to-sidebar-800 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md mx-4">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl dark:text-surface-200 font-display font-bold text-white">Vitamin Shots</h1>
          <p className="text-sidebar-400 text-sm mt-1">Finance Minister</p>
        </div>
        <div className="bg-white dark:bg-surface-800/10 backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-modal">
          {step === 'credentials' && (
            <form onSubmit={handleLogin} className="space-y-5">
              <div><label className="block text-xs font-semibold text-sidebar-300 mb-1.5 uppercase tracking-wider">Email</label>
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required className="w-full px-4 py-3 bg-white dark:bg-surface-800/10 border border-white/10 rounded-xl text-white placeholder-sidebar-500 focus:outline-none focus:ring-2 focus:ring-brand-400/50 text-sm" placeholder="your@email.com" /></div>
              <div><label className="block text-xs font-semibold text-sidebar-300 mb-1.5 uppercase tracking-wider">Password</label>
                <input type="password" value={password} onChange={e=>setPassword(e.target.value)} required className="w-full px-4 py-3 bg-white dark:bg-surface-800/10 border border-white/10 rounded-xl text-white placeholder-sidebar-500 focus:outline-none focus:ring-2 focus:ring-brand-400/50 text-sm" /></div>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div>
                <input type="text" value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,'').slice(0,6))} required maxLength={6} placeholder="000000"
                  className="w-full px-4 py-4 bg-white dark:bg-surface-800/10 border border-white/10 rounded-xl text-white text-center text-2xl font-mono tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-brand-400/50" autoFocus />
              </div>
              {error && <div className="text-red-400 text-sm bg-red-500/10 px-3 py-2 rounded-xl">{error}</div>}
              {info && <div className="text-emerald-400 text-sm bg-emerald-500/10 px-3 py-2 rounded-xl">{info}</div>}
          ...
          {step === 'reset' && (
            <form onSubmit={handleReset} className="space-y-5">
              ...
              <div><input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="Email" className="w-full px-4 py-3 bg-white dark:bg-surface-800/10 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-brand-400/50 text-sm" /></div>
              {resetStep === 'otp' && <>
                <div><input type="text" value={resetOtp} onChange={e=>setResetOtp(e.target.value.replace(/\D/g,'').slice(0,6))} required maxLength={6} placeholder="000000" className="w-full px-4 py-4 bg-white dark:bg-surface-800/10 border border-white/10 rounded-xl text-white text-center text-2xl font-mono tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-brand-400/50" /></div>
                <div><input type="password" value={newPass} onChange={e=>setNewPass(e.target.value)} required minLength={8} placeholder="New password (8+ chars)" className="w-full px-4 py-3 bg-white dark:bg-surface-800/10 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-brand-400/50 text-sm" /></div>
              </>}
              {error && <div className="text-red-400 text-sm bg-red-500/10 px-3 py-2 rounded-xl">{error}</div>}
              {info && <div className="text-emerald-400 text-sm bg-emerald-500/10 px-3 py-2 rounded-xl">{info}</div>}
