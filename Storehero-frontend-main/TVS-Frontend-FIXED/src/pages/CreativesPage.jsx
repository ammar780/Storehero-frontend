...
export default function CreativesPage() {
  ...
  const { data, loading , error } = useApi('/marketing/detailed', { period }, [period]);
            ...
            {campaigns.map((c,i) => (
              <div key={i} className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><div className="text-[10px] text-surface-400 uppercase">ROAS</div><div className={`font-mono text-sm font-bold ${+c.roas>=3?'text-emerald-600':+c.roas>=1?'text-blue-600':'text-red-600'}`}>{((+c.roas)||0).toFixed(1)}x</div></div>
