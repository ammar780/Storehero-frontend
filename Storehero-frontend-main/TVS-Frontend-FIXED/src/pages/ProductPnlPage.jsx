...
export default function ProductPnlPage() {
  ...
  const { data, loading , error } = useApi('/analytics/product-pnl', { period }, [period]);
  ...
  return (
    <div>
      ...
      {loading ? <><SkeletonCards count={4}/><SkeletonTable/></> : (<>
        ...
        {products.length > 0 ? (
          <DataTable columns={[
            {key:'name',label:'Product',render:v=><span className="font-semibold text-sm dark:text-surface-200">{v}</span>},
