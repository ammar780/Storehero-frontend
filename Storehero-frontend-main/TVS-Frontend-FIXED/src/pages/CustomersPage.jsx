...
export default function CustomersPage() {
  ...
  const { data, loading , error } = useApi('/analytics/customer-segments', {}, []);
  ...
  return (
    <div>
      ...
      {loading ? <><SkeletonCards count={4}/><SkeletonTable/></> : (!data?.totalCustomers ? (
        <div className="text-center py-16 text-surface-400 dark:text-surface-500"><Users size={48} className="mx-auto mb-4 opacity-30"/><div className="text-lg font-display font-bold mb-2">No customer data yet</div><p className="text-sm">Sync WooCommerce to import customers and see RFM segmentation.</p></div>
      ) : (<>
          ...
          {pieData.length > 0 && (
            <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <PieChart><Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label={({name,percent})=>`${name} ${((percent*100)||0).toFixed(0)}%`}>
