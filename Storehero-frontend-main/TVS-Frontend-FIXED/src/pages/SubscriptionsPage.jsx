...
export default function SubscriptionsPage() {
  ...
  const { data, loading , error } = useApi('/analytics/subscriptions', { period }, [period]);
  const { data: mrrData } = useApi('/analytics/mrr-waterfall', { period }, [period]);
    ...
    <div>
      ...
      {loading ? <><SkeletonCards count={6}/><SkeletonTable/></> : (<>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6 stagger">
          ...
          <MetricCard label="One-Time AOV" value={s.onetime_aov} icon={ShoppingCart} color="bg-surface-50 dark:bg-surface-700 text-surface-600" />
        </div>
        ...
        {/* MRR Waterfall */}
        {mrrData?.monthly?.length > 0 && (
          <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5 mb-6">
            <h3 className="font-display font-bold text-sm mb-4">MRR Waterfall</h3>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={mrrData.monthly}>
                <XAxis dataKey="month" tick={{fontSize:10}} tickFormatter={v=>v?.slice(5,7)}/>
                <YAxis tick={{fontSize:10}} tickFormatter={v=>'$'+fmt.compact(v)}/><Tooltip formatter={v=>fmt.currency(v)}/><Legend/>
                <Bar dataKey="new_mrr" fill="#22c55e" name="New MRR" stackId="a" radius={[4,4,0,0]}/>
