...
function Tab({ tabs, active, onChange }) {
  return (
    <div className="inline-flex bg-white dark:bg-surface-800 rounded-xl border border-surface-200 dark:border-surface-700 p-1 shadow-sm mb-6 flex-wrap gap-0.5">
      {(tabs||[]).map(t => (
...
const INTEGRATION_FIELDS = {
  woocommerce: {
    ...
    extraSync: { label: 'Sync Stock', endpoint: '/sync/stock' },
  },
  meta_ads: {
  ...
  google_ads: {
    ...
    fields: [
      { key: 'developer_token', label: 'Developer Token', placeholder: 'From Google Ads API Center', type: 'password' },
      { key: 'client_id', label: 'OAuth Client ID', placeholder: 'xxxxx.apps.googleusercontent.com', type: 'text' },
      { key: 'client_secret', label: 'OAuth Client Secret', type: 'password' },
      { key: 'refresh_token', label: 'Refresh Token', placeholder: 'Generated via OAuth flow', type: 'password' },
      { key: 'customer_id', label: 'Customer ID (xxx-xxx-xxxx)', placeholder: '123-456-7890', type: 'text' },
    ],
  ...
  tiktok_ads: {
    ...
    fields: [
      { key: 'access_token', label: 'Access Token', placeholder: 'From TikTok Business Center', type: 'password' },
  ...
  paypal: {
    label: 'PayPal', color: 'bg-blue-800',
    fields: [
      { key: 'client_id', label: 'Client ID', placeholder: 'AV4x...', type: 'text' },
      { key: 'secret', label: 'Secret', placeholder: 'EGn8...', type: 'password' },
      { key: 'sandbox', label: 'Sandbox Mode? (yes/no)', placeholder: 'no', type: 'text' },
    ],
    syncEndpoint: '/sync/paypal',
  },
  amazon_marketplace: {
  ...
  emailit_smtp: {
    label: 'EmailIt (SMTP — transactional)', color: 'bg-rose-600',
    fields: [
      { key: 'smtp_host', label: 'SMTP Host', placeholder: 'smtp.emailit.com', type: 'text' },
    ...
    info: 'Your website transactional email SMTP provider (OTP, password resets). Used as fallback if Resend is not configured.',
    syncEndpoint: '/settings/emailit-test',
  },
  emailit: {
    label: 'Emailit (Campaigns & Tracking)', color: 'bg-rose-700',
    fields: [
      { key: 'api_key', label: 'API Key', placeholder: 'Emailit dashboard → API Keys', type: 'password' },
      { key: 'api_base_url', label: 'API Base URL', placeholder: 'https://api.emailit.com/v1', type: 'url' },
    ],
    info: 'Powers the Emailit tab: campaign/autoresponder sync + delivered/bounced/spam/open/click tracking. Also set EMAILIT_WEBHOOK_SECRET in Railway for real-time events.',
    syncEndpoint: '/sync/emailit',
  },
  enginemailer: {
    label: 'Enginemailer (Campaigns)', color: 'bg-purple-700',
    fields: [
      { key: 'api_key', label: 'API Key', type: 'password' },
      { key: 'api_base_url', label: 'API Base URL', placeholder: 'https://api.enginemailer.com/v2', type: 'url' },
    ],
    syncEndpoint: '/sync/enginemailer',
  },
  resend: {
    label: 'Resend (Email)', color: 'bg-black',
    fields: [
      { key: 'api_key', label: 'API Key', placeholder: 're_xxxxx...', type: 'password' },
      { key: 'from_email', label: 'From Email', placeholder: 'noreply@thevitaminshots.com', type: 'text' },
    ],
    info: 'Used for OTP login codes, password resets, and weekly reports. Get your key at resend.com/api-keys',
  },
  slack: {
    label: 'Slack Notifications', color: 'bg-emerald-700',
    fields: [
      { key: 'webhook_url', label: 'Webhook URL', placeholder: 'https://hooks.slack.com/services/...', type: 'text' },
    ],
    info: 'Get a webhook URL from Slack: Apps > Incoming Webhooks > Add New',
  },


  hub_botdolphin: { label: 'Hub: BotDolphin', color: 'bg-blue-500', fields: [{ key: 'url', label: 'Backend URL', placeholder: 'https://app.botdolphin.com' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'AI chatbot SaaS' },
  hub_webcomplifier: { label: 'Hub: WebComplifier', color: 'bg-purple-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'WCAG accessibility scanner' },
  hub_sitescannify: { label: 'Hub: Sitescannify', color: 'bg-orange-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'Conversion audit tool' },
  hub_audience_iq: { label: 'Hub: Audience IQ', color: 'bg-pink-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'CLV + Meta audiences' },
  hub_zerorank: { label: 'Hub: ZeroRank', color: 'bg-red-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'AI search visibility' },
  hub_gsc_reporter: { label: 'Hub: GSC Reporter', color: 'bg-green-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'Search Console reports' },
  hub_amazon_tool: { label: 'Hub: Amazon Tool', color: 'bg-amber-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'Amazon seller research' },
  hub_dispute_shield: { label: 'Hub: Dispute Shield', color: 'bg-red-600', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'Chargeback management' },
  hub_wellness_app: { label: 'Hub: Wellness App', color: 'bg-emerald-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'iOS/Android app' },
  hub_admin_panel: { label: 'Hub: Admin Panel', color: 'bg-gray-500', fields: [{ key: 'url', label: 'Backend URL' },{ key: 'api_key', label: 'Hub API Key', type: 'password' }], info: 'App admin dashboard' },
  referly: {
    label: 'Referly.so (Affiliates)', color: 'bg-violet-600',
    fields: [
      { key: 'api_key', label: 'API Key (Bearer Token)', placeholder: 'Your Referly API key', type: 'password' },
    ],
    syncEndpoint: '/sync/referly',
  },
  pinterest_ads: {
    label: 'Pinterest Ads', color: 'bg-red-700',
    fields: [
      { key: 'access_token', label: 'Access Token', placeholder: 'From Pinterest Business → Apps', type: 'password' },
      { key: 'ad_account_id', label: 'Ad Account ID', placeholder: 'Your Pinterest ad account ID', type: 'text' },
    ],
    syncEndpoint: '/sync/ad-spend', syncBody: { platform: 'pinterest' },
  },
  microsoft_ads: {
...
function IntegrationCard({ integration, onSave, onSync, syncing }) {
  const toast = useToast(); // #3 Fix: toast was missing
  ...
  if (!def) {
    ...
    return (
      <div className={`bg-white dark:bg-surface-800 rounded-2xl border p-5 transition-all hover:shadow-cardHover ${integration.is_connected ? 'border-emerald-200' : 'border-surface-100'}`}>
  ...
  return (
    <div className={`bg-white dark:bg-surface-800 rounded-2xl border overflow-hidden transition-all hover:shadow-cardHover ${integration.is_connected ? 'border-emerald-200' : 'border-surface-100'}`}>
      {/* Header */}
      <button onClick={() => setExpanded(!expanded)} className="w-full px-5 py-4 flex items-center justify-between hover:bg-surface-50 dark:bg-surface-700/50 transition-colors">
              ...
              <div key={f.key}>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 border border-surface-200 dark:border-surface-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-400/30 pr-10 font-mono text-xs"
                  />
                  {isSecret && (
...
export default function SettingsPage() {
  ...
  const { data: integrations, loading: li, refetch: ri , error } = useApi('/settings/integrations');
  ...
  const delCost = async (id) => { if(!window.confirm('Delete this fixed cost?'))return; await api.delete('/settings/fixed-costs/' + id); rc(); toast.success('Cost deleted'); };

  const tabs = [
    { key: 'integrations', label: 'Integrations' },
    { key: 'ceo', label: 'CEO Report' },
  ...
  const sortedIntegrations = [...(integrations || []).filter(i => !String(i.platform).startsWith('hub_'))].sort((a, b) => {
    ...
    <div>
      ...
      {tab === 'costs' && (
        <div>
          ...
          {addingCost && (
            <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5 mb-4 animate-scale-in">
              <div className="flex flex-wrap gap-3 items-end">
                <div className="flex-1 min-w-[150px]"><label className="block text-xs font-semibold text-surface-500 mb-1">Name</label><input value={costForm.name} onChange={e => setCostForm({...costForm,name:e.target.value})} className="w-full px-3 py-2 border border-surface-200 dark:border-surface-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-400/30" placeholder="e.g. Shopify, Hosting" /></div>
                <div className="w-32"><label className="block text-xs font-semibold text-surface-500 mb-1">Monthly $</label><input type="number" value={costForm.amount_monthly} onChange={e => setCostForm({...costForm,amount_monthly:e.target.value})} className="w-full px-3 py-2 border border-surface-200 dark:border-surface-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-400/30" /></div>
                <div className="w-32"><label className="block text-xs font-semibold text-surface-500 mb-1">Category</label><input value={costForm.category} onChange={e => setCostForm({...costForm,category:e.target.value})} className="w-full px-3 py-2 border border-surface-200 dark:border-surface-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-400/30" placeholder="SaaS" /></div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {(costsData?.costs||[]).map(c => (
                    <tr key={c.id} className="border-b border-surface-50 hover:bg-surface-50 dark:bg-surface-700/50">
      ...
      {tab === 'reports' && (
        <div className="space-y-6 stagger">
          <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-4 sm:p-6">
            ...
            <div className="bg-surface-50 dark:bg-surface-700 rounded-xl p-3 text-xs text-surface-500 mb-4">Configure SMTP settings as env vars on Railway: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, EMAIL_FROM</div>
        ...
        <div>
          {la ? <SkeletonTable /> : (
            <div className="space-y-3">
              {(alerts||[]).map(a => (
                <div key={a.id} className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5 flex items-center justify-between hover:shadow-cardHover transition-shadow">
                  <div className="flex items-center gap-3">
      ...
      {tab === 'ceo' && <CeoReportTab />}
    </div>
  );
}

function CeoReportTab() {
  const toast = useToast();
  ...
  useEffect(() => {
    api.get('/settings/ceo-report')
      .then(({ data }) => { setCfg(data); setRecipients((data.recipients || []).join(', ')); })
      .catch(() => setCfg({ recipients: [], daily_enabled: true, weekly_enabled: true, meta_daily_enabled: true, daily_time: '08:00', weekly_time: '08:00', audit_time: '14:00', timezone: 'America/New_York' }));
  }, []);
  ...
  const save = async () => {
    setSaving(true);
    try {
      const { data } = await api.put('/settings/ceo-report', { ...cfg, recipients });
      setCfg(data); setRecipients((data.recipients || []).join(', '));
      toast.success('CEO report settings saved');
    } catch (e) { toast.error(e.response?.data?.error || e.message); }
    finally { setSaving(false); }
  };
  ...
  const sendTest = async (period) => {
    setTesting(period);
    try { const { data } = await api.post('/hub/send-report', { period }); toast.success(period + ' report sent to ' + data.sent + ' recipient(s)'); }
    catch (e) { toast.error(e.response?.data?.error || e.message); }
    finally { setTesting(null); }
  };

  if (!cfg) return <SkeletonTable />;

  const TIMES = Array.from({ length: 24 }, (_, h) => String(h).padStart(2, '0') + ':00');
  const TZS = ['America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles', 'UTC', 'Europe/London', 'Asia/Karachi', 'Asia/Dubai', 'Asia/Kolkata', 'Australia/Sydney'];
  const Toggle = ({ on, onClick }) => (
    <button onClick={onClick} className={`relative w-11 h-6 rounded-full transition-colors ${on ? 'bg-emerald-500' : 'bg-surface-300'}`}><span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${on ? 'translate-x-5' : 'translate-x-0.5'}`} /></button>
  );
  const selCls = 'w-full px-3 py-2 rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 text-sm';

  return (
    <div className="space-y-6 stagger">
      <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-4 sm:p-6">
        <div className="flex items-center gap-3 mb-1">
          <div className="w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center"><Mail size={20} className="text-brand-700" /></div>
          <div>
            <h3 className="font-display font-bold text-surface-800 dark:text-surface-100">CEO Email Report</h3>
            <p className="text-xs text-surface-400">AI brief across the store and every connected product — situation analysis plus prioritized actions.</p>
          </div>
        </div>
        <label className="block text-xs font-semibold text-surface-500 mt-4 mb-1">Recipient email(s)</label>
        <input value={recipients} onChange={e => setRecipients(e.target.value)} placeholder="ceo@thevitaminshots.com, you@example.com" className="w-full px-4 py-2.5 rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        <p className="text-[11px] text-surface-400 mt-1">Separate multiple addresses with commas. Leave blank to pause all CEO reports.</p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
          <div className="flex items-center justify-between mb-3">
            <div><div className="font-display font-bold text-sm dark:text-surface-100">Daily Brief</div><div className="text-[11px] text-surface-400">Every day (except Saturday)</div></div>
            <Toggle on={cfg.daily_enabled} onClick={() => set('daily_enabled', !cfg.daily_enabled)} />
          </div>
          <label className="block text-xs font-semibold text-surface-500 mb-1">Send time</label>
          <select value={cfg.daily_time} onChange={e => set('daily_time', e.target.value)} className={selCls}>{TIMES.map(t => <option key={t} value={t}>{t}</option>)}</select>
        </div>
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
          <div className="flex items-center justify-between mb-3">
            <div><div className="font-display font-bold text-sm dark:text-surface-100">Weekly Report</div><div className="text-[11px] text-surface-400">Saturdays — after the Friday re-audit</div></div>
            <Toggle on={cfg.weekly_enabled} onClick={() => set('weekly_enabled', !cfg.weekly_enabled)} />
          </div>
          <label className="block text-xs font-semibold text-surface-500 mb-1">Send time (Saturday)</label>
          <select value={cfg.weekly_time} onChange={e => set('weekly_time', e.target.value)} className={selCls}>{TIMES.map(t => <option key={t} value={t}>{t}</option>)}</select>
        </div>
      </div>

      <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
        <div className="flex items-center justify-between">
          <div><div className="font-display font-bold text-sm dark:text-surface-100">📘 Daily Meta Ads email</div><div className="text-[11px] text-surface-400">A separate Meta-only report sent each morning alongside the daily brief</div></div>
          <Toggle on={cfg.meta_daily_enabled} onClick={() => set('meta_daily_enabled', !cfg.meta_daily_enabled)} />
        </div>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
          <label className="block text-xs font-semibold text-surface-500 mb-1">Friday auto-audit time</label>
          <select value={cfg.audit_time} onChange={e => set('audit_time', e.target.value)} className={selCls}>{TIMES.map(t => <option key={t} value={t}>{t}</option>)}</select>
          <p className="text-[11px] text-surface-400 mt-1">Every product is re-audited Friday; results feed Saturday's report.</p>
        </div>
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 p-5">
          <label className="block text-xs font-semibold text-surface-500 mb-1">Timezone</label>
          <select value={cfg.timezone} onChange={e => set('timezone', e.target.value)} className={selCls}>{TZS.map(t => <option key={t} value={t}>{t}</option>)}</select>
        </div>
      </div>

      <div className="flex flex-wrap gap-2">
        <button onClick={save} disabled={saving} className="flex items-center gap-2 px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-sm font-semibold disabled:opacity-50">{saving ? <RefreshCw size={14} className="animate-spin" /> : <Save size={14} />} Save Settings</button>
        <button onClick={() => sendTest('daily')} disabled={!!testing} className="flex items-center gap-2 px-4 py-2.5 bg-surface-200 dark:bg-surface-700 rounded-xl text-sm font-semibold disabled:opacity-50 dark:text-surface-200">{testing === 'daily' ? <RefreshCw size={14} className="animate-spin" /> : <Mail size={14} />} Send test daily</button>
        <button onClick={() => sendTest('weekly')} disabled={!!testing} className="flex items-center gap-2 px-4 py-2.5 bg-surface-200 dark:bg-surface-700 rounded-xl text-sm font-semibold disabled:opacity-50 dark:text-surface-200">{testing === 'weekly' ? <RefreshCw size={14} className="animate-spin" /> : <Mail size={14} />} Send test weekly</button>
        <button onClick={() => sendTest('meta-daily')} disabled={!!testing} className="flex items-center gap-2 px-4 py-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-xl text-sm font-semibold disabled:opacity-50">{testing === 'meta-daily' ? <RefreshCw size={14} className="animate-spin" /> : <Mail size={14} />} Send test Meta</button>
      </div>
      <div className="bg-surface-50 dark:bg-surface-700/40 rounded-xl p-3 text-[11px] text-surface-500">Email delivery needs RESEND_API_KEY or SMTP_* env vars on Railway. Reports pull each product's audit using the Hub keys set under Integrations.</div>
    </div>
  );
}
