...
import { Package, AlertTriangle, Edit3, Check, X, Boxes, RefreshCw } from 'lucide-react';
...
export default function InventoryPage() {
  ...
  const syncStock = async () => { setSyncing(true); try { const{data:d}=await api.post('/sync/stock'); toast.success(d.message||'Stock synced'); refetch(); } catch(e){toast.error(e.response?.data?.error||e.message);} finally{setSyncing(false);} };
  const [alerting, setAlerting] = useState(false);
  const sendStockAlert = async () => { setAlerting(true); try { const{data:d}=await api.post('/inventory/ai-alert'); toast.success(d.message||'AI stock alert sent to '+d.products+' products'); } catch(e){toast.error(e.response?.data?.error||e.message);} finally{setAlerting(false);} };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4"><PageHeader title="Inventory" subtitle="Stock levels, reorder alerts, and inventory valuation"><button onClick={syncStock} disabled={syncing} className="flex items-center gap-2 px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-sm font-semibold disabled:opacity-50"><RefreshCw size={14} className={syncing?"animate-spin":""}/>{syncing?"Syncing...":"Sync Stock"}</button><button onClick={sendStockAlert} disabled={alerting} className="flex items-center gap-2 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-sm font-semibold disabled:opacity-50"><RefreshCw size={14} className={alerting?"animate-spin":""}/>{alerting?"Sending...":"AI Stock Alert"}</button></PageHeader></div>
      ...
      {loading ? <SkeletonTable /> : products.length > 0 ? (
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 overflow-hidden">
          <table className="w-full">
            <thead><tr className="border-b border-surface-100 bg-surface-50 dark:bg-surface-700/50 dark:bg-surface-900/50">
            ...
            <tbody>
              {(products||[]).map(p => {
                ...
                return (
                  <tr key={p.id} className={`border-b border-surface-50 ${danger ? 'bg-red-50/50 dark:bg-red-900/10' : 'hover:bg-surface-50 dark:bg-surface-700/50'}`}>
                    ...
                    <td className="px-4 py-3 font-mono text-sm">{((+p.avg_daily_sales)||0).toFixed(1)}/day</td>
