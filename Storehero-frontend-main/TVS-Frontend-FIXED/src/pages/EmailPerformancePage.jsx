...
export default function EmailPerformancePage() {
  ...
  const { data, loading , error } = useApi('/analytics/email-performance', { period }, [period]);
    ...
    <div>
      ...
      {loading ? <><SkeletonCards count={6}/><SkeletonTable/></> : (<>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6 stagger">
          <MetricCard label="Emails Sent" value={s.total_sends} format="number" icon={Send} color="bg-surface-50 dark:bg-surface-700 text-surface-600" />
        ...
        {tab === 'campaigns' && (<div className="space-y-6">
          {data?.byCampaign?.length > 0 ? (<>
            ...
            <DataTable columns={[
              ...
              {key:'delivery_rate',label:'Delivered %',align:'right',render:v=><span className="font-mono text-emerald-600">{v}%</span>},
              {key:'bounce_rate',label:'Bounce %',align:'right',render:v=><span className={`font-mono ${v>2?'text-red-500 font-bold':''}`}>{v}%</span>},
              {key:'spam_rate',label:'Spam %',align:'right',render:v=><span className={`font-mono ${v>=0.1?'text-red-500 font-bold':''}`}>{v}%</span>},
              ...
              {key:'unique_clickers',label:'Clicked',align:'right',render:v=>fmt.number(v)},
              {key:'click_rate',label:'Click %',align:'right',render:v=><span className="font-mono">{v}%</span>},
              {key:'click_to_open_rate',label:'CTOR %',align:'right',render:v=><span className="font-mono text-surface-400">{v}%</span>},
        ...
        {tab === 'deliverability' && (<div className="space-y-6">
          <div className="bg-gradient-to-r from-surface-50 to-blue-50 dark:from-surface-800 dark:to-blue-900/20 rounded-2xl border border-surface-200 dark:border-surface-700 p-4 sm:p-6">
